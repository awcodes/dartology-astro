    ...
    <section v-if="!cards.length">
      ...
      <ul class="mt-6 divide-y divide-gray-700">
        <li v-for="(skill, key) in skills" :key="key">
          <button
            class="flex items-center w-full py-3 text-left group"
            @click="setSkill(key)"
          >
            <span class="flex-1 block">
              ...
              <p class="text-sm group-hover:text-secondary-300">
                Outs from {{ skill.min }} - {{ skill.max }}
              </p>
            </span>
            <svg
    ...
    <section v-if="cards.length">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm">
            Outs from {{ skills[skill].min }} - {{ skills[skill].max }}
          </p>
        </div>
        <button @click="reset" class="py-1 text-sm btn secondary">
          Start Over
        </button>
      </div>
      <div class="mt-6">
      ...
      <div ref="deckNav" class="flex items-center justify-between mt-4">
        ...
        <button class="btn" @click="showAnswer" :disabled="answered">
          Show Answer
        </button>
        <button
          class="px-2 rounded-full btn orange"
      ...
      <div class="p-6 mt-6 space-y-4 bg-gray-800 rounded" v-show="answered">
        <div
          v-if="cards[current].primary.length"
          class="text-xl px-4 py-2 text-center bg-gray-900 rounded mx-auto"
        >
          {{ cards[current].primary }}
        </div>
        <p
          v-if="cards[current].details.length"
          v-html="cards[current].details"
          class="text-sm"
        ></p>
        <p v-if="cards[current].secondary.length" class="text-xs">
          Secondary Out: {{ cards[current].secondary }}
        </p>
        <p v-if="cards[current].tertiary.length" class="text-xs">
          Tertiary Out: {{ cards[current].tertiary }}
        </p>
      </div>
    </section>
  </div>
  ...
  methods: {
    ...
    setSkill(level) {
      ...
      this.cards = this.outs.filter((card) => {
        return (
          card.score >= this.skills[level].min &&
          card.score <= this.skills[level].max
        );
      });
